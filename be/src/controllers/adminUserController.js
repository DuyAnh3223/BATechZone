...
export const listUsers = async (req, res) => {
    try {
        ...
        const { users, total } = await User.listAndCount({ search, role, is_active, page: parseInt(page), pageSize: parseInt(pageSize) });
        return res.json({ success: true, data: users.map(u => u.toJSON ? u.toJSON() : {
            user_id: u.user_id,
            username: u.username,
            email: u.email,
