...
const payments = [
  {
    ...
    paid_at: '2024-04-18 10:21',
    created_at: '2024-04-18 10:10',
  },
  {
    payment_id: 2,
    order_id: 2,
    ...
    paid_at: '2024-04-21 13:05',
    created_at: '2024-04-21 12:55',
  },
  {
    payment_id: 3,
    order_id: 3,
...
const statusClass = (s) => (
  ...
  s === 'refunded' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'
);
...
const AdminPayment = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter(p => {
      ...
      const matchText = !q || orderNumber.includes(q) || tx.includes(q) || String(p.order_id).includes(q);
  ...
  return (
    <section>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Quản lý thanh toán (Payments)</h1>
        <button className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90 text-white px-6 py-2 rounded-lg font-semibold shadow transition">+ Tạo giao dịch</button>
      </div>
    {/* Bộ lọc nhanh */}
    <div className="mb-3 flex flex-wrap items-center gap-2">
      <input value={search} onChange={(e)=>{ setSearch(e.target.value); setPage(1); }} className="border rounded px-3 py-2 w-full md:w-72" placeholder="Tìm theo mã đơn/Order ID/Transaction ID..." />
      <select value={method} onChange={(e)=>{ setMethod(e.target.value); setPage(1); }} className="border rounded px-3 py-2">
      ...
      <div className="overflow-x-auto rounded-xl bg-white shadow pb-2">
        <table className="min-w-[1200px] w-full text-left">
          ...
          {paginated.map((p) => (
              <tr key={p.payment_id} className="hover:bg-blue-50 transition">
                <td className="px-4 py-3 font-medium text-gray-800">{p.payment_id}</td>
                <td className="px-4 py-3">{p.order_id}</td>
                <td className="px-4 py-3">{translatePaymentMethod(p.payment_method)}</td>
                <td className="px-4 py-3"><span className={`px-3 py-0.5 rounded-full text-xs font-semibold ${statusClass(p.payment_status)}`}>{translatePaymentStatus(p.payment_status)}</span></td>
                ...
                <td className="px-4 py-3 flex gap-2 whitespace-nowrap">
                  <button onClick={() => openDetail(p)} className="px-3 py-1 bg-indigo-100 hover:bg-indigo-200 text-indigo-600 rounded text-xs font-medium">Chi tiết</button>
                  <button className="px-3 py-1 bg-pink-100 hover:bg-pink-200 text-pink-600 rounded text-xs font-medium">Hoàn tiền</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex items-center justify-between px-3 py-2 text-sm text-gray-600">
        <div>Tổng: <span className="font-medium text-gray-800">{filtered.length}</span> giao dịch — Trang {currentPage}/{totalPages}</div>
        ...
        <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center transition">
          <div className="bg-white rounded-xl shadow-xl w-[94vw] max-w-4xl p-6 relative z-50">
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-4 text-sm">
              <div className="border rounded p-3"><div className="text-gray-500">Trạng thái đơn</div><div className={`font-semibold px-2 py-1 rounded inline-block ${getOrderStatusClass(selectedOrder.order_status)}`}>{translateOrderStatus(selectedOrder.order_status)}</div></div>
              <div className="border rounded p-3"><div className="text-gray-500">Thanh toán</div><div className={`font-semibold px-2 py-1 rounded inline-block ${getPaymentStatusClass(selectedOrder.payment_status)}`}>{translatePaymentStatus(selectedOrder.payment_status)}</div></div>
